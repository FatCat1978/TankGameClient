...
public class Bullet {
	...
	private float bulletSpeed = 256*2;	
	...
	public Bullet(MyGdxGame game, tankAppClient theClient, Vector2 newPos, float theAngle) 	// Create/instantiate 
	{
		this.theClient = theClient;
		position = new Vector2();
		direction = new Vector2();
		this.game = game; //ensures that all data from the class MyGdxGame is identical to the data of the same type here
		bulletTexture = game.manager.get("Tank_Bullet_V2.png", Texture.class);
	...
	public void setBulletPosition(float x, float y) {
		position.x = x;
		position.y = y;
		theClient.setBulletData(true, position.x, position.y, bulletAngle);
	}
